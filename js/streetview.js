...
async function fetchKartaView(lat, lon) {
  for (const baseUrl of KARTAVIEW_URLS) {
    try {
      const url = `${baseUrl}?lat=${lat}&lng=${lon}&radius=0.1`;
      ...
      if (!imgUrl) continue;
      return { url: imgUrl, heading: parseFloat(photo.heading) || 0, source: 'kartaview' };
    } catch { continue; }
  }
  return null;
}
...
async function fetchMapillary(lat, lon, heading) {
  ...
  try {
    ...
    const url = `${MAPILLARY_URL}?fields=id,geometry,thumb_1024_url,computed_compass_angle&bbox=${bbox}&limit=15&access_token=${token}`;
    const resp = await fetch(url, { signal: AbortSignal.timeout(5000) });
...
class StreetView {
  ...
  // wrapEl: il contenitore della street view — reso trasparente quando no image
  init(imgElement, noImgElement, wrapElement = null) {
  ...
  async update({ current_point, heading }) {
    ...
    if (img) {
      ...
      if (!this._cache.has(nextKey)) {
        ...
        fetchKartaView(nextLat, nextLon).then(ni => this._cache.set(nextKey, ni || null));
      }
    } else {
      this._noImageStreak++;
      if (this._noImageStreak >= 2) {
        // Rendi la wrap trasparente → mostra MapLibre city view sotto
